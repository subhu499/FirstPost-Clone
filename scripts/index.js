let india_news=async()=>{
    try {        
    let res_mid=await fetch(`https://firstpost-masai-server.herokuapp.com/HOME-PAGE`);
    let data=await res_mid.json();
    console.log(data);
    append_mid(data);
    let res_right=await fetch(`https://firstpost-masai-server.herokuapp.com/HOME-PAGE`);
