let india_news = async () => {
  try {
    let res_mid = await fetch(
      `https://firstpost-masai-server.herokuapp.com/INDIA`
    );
    let data = await res_mid.json();
    console.log(data);
    append_mid(data);
    slider(data);
    let res_right = await fetch(
      `https://firstpost-masai-server.herokuapp.com/INDIA`
    );
...
const append_mid = (data) => {
  ...
  data.forEach((ele) => {
    ...
    shareimg.src =
      "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQRS73qHKRnu8aEj-j6dWABcHumWPOGDctcIw&usqp=CAU";
...
const subscribe_btn = () => {
  ...
  if (subemail != null) {
    document.querySelector("#subscribed").innerText =
